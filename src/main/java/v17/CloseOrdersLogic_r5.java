 ...
 * 返済注文約定情報を管理する。
 */
public class CloseOrdersLogic_r5 {
	 ...
	 */
	private static final String TXT_FILEPATH = DIRPATH + "CloseOrdersLogic_r5.txt";
	/**
	 * 返済注文情報の削除情報を保存したファイルパス。存在しなければ生成される。
	 */
	private static final String DEL_FILEPATH = DIRPATH + "CloseOrdersLogic_r5.del";
	/**
	 * 返済注文情報ログのファイルパス。存在しなければ生成される。
	 */
	private static final String LOG_FILEPATH = DIRPATH + "CloseOrdersLogic_r5.log";
	/**
	 * 返済注文情報ファイルのカラム数。
	 */
	 ...
	 */
	public CloseOrdersLogic_r5(String X_API_KEY) {
	...
	public void execute() throws ApiException {
		...
		System.out.println("CloseOrdersLogic_r5.execute(): response.size=" + response.size());
	...
	private void readOrders() {
		...
		System.out.println("CloseOrdersLogic_r5.readOrders(): orderMap.size=" + orderMap.size());
		for (String key : orderMap.keySet()) {
			CloseInfo ci = orderMap.get(key);
	...
	private void deleteOrders() {
		if (orderKeySet.size() > 0) {
			try (PrintWriter pw = FileUtil.writer(DEL_FILEPATH, FileUtil.UTF8, true)) {
				System.out.println("CloseOrdersLogic_r5.deleteOrders(): orderKeySet.size=" + orderKeySet.size());
	...
	public void writeOrders() {
		System.out.println("CloseOrdersLogic_r5.writeOrders(): orderMap.size=" + orderMap.size());
