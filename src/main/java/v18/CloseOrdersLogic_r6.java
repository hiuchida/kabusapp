 ...
 * 返済注文約定情報を管理する。
 */
public class CloseOrdersLogic_r6 {
	 ...
	 */
	private static final String TXT_FILEPATH = DIRPATH + "CloseOrdersLogic_r6.txt";
	/**
	 * 返済注文情報の削除情報を保存したファイルパス。存在しなければ生成される。
	 */
	private static final String DEL_FILEPATH = DIRPATH + "CloseOrdersLogic_r6.del";
	/**
	 * 返済注文情報ログのファイルパス。存在しなければ生成される。
	 */
	private static final String LOG_FILEPATH = DIRPATH + "CloseOrdersLogic_r6.log";
	/**
	 * 返済注文情報ファイルのカラム数。
	 */
	 ...
	 */
	public CloseOrdersLogic_r6(String X_API_KEY) {
	...
	public void execute() throws ApiException {
		...
		System.out.println("CloseOrdersLogic_r6.execute(): response.size=" + response.size());
	...
	private void readOrders() {
		...
		System.out.println("CloseOrdersLogic_r6.readOrders(): orderMap.size=" + orderMap.size());
		for (String key : orderMap.keySet()) {
			CloseInfo ci = orderMap.get(key);
	...
	private void deleteOrders() {
		if (orderKeySet.size() > 0) {
			try (PrintWriter pw = FileUtil.writer(DEL_FILEPATH, FileUtil.UTF8, true)) {
				System.out.println("CloseOrdersLogic_r6.deleteOrders(): orderKeySet.size=" + orderKeySet.size());
	...
	public void writeOrders() {
		System.out.println("CloseOrdersLogic_r6.writeOrders(): orderMap.size=" + orderMap.size());
